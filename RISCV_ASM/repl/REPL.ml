...
let pp_instruction_3reg_helper ppf instruction r1 r2 r3 =
  Format.fprintf ppf "%s %s, %s, %s" instruction (pp_register r1) (pp_register r2) (pp_register r3)
...
let pp_instruction_2reg_1imm_helper ppf instruction r1 r2 addr =
  Format.fprintf ppf "%s %s,%s,%s" instruction (pp_register r1) (pp_register r2) (pp_address addr)

let pp_instruction_2reg_1offset_helper ppf instruction r1 r2 addr =
  Format.fprintf ppf "%s %s,%s(%s)" instruction (pp_register r1) (pp_address addr) (pp_register r2)
...
let pp_instruction ppf = function
  ...
  | Addi (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "addi" rd rs1 (Address12 imm)
  | Xori (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "xori" rd rs1 (Address12 imm)
  | Ori (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "ori" rd rs1 (Address12 imm)
  | Andi (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "andi" rd rs1 (Address12 imm)
  | Slli (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "slli" rd rs1 (Address12 imm)
  | Srli (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "srli" rd rs1 (Address12 imm)
  | Srai (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "srai" rd rs1 (Address12 imm)
  | Slti (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "slti" rd rs1 (Address12 imm)
  | Sltiu (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "sltiu" rd rs1 (Address12 imm)
  | Lb (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "lb" rd rs1 (Address12 imm)
  | Lh (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "lh" rd rs1 (Address12 imm)
  | Lw (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "lw" rd rs1 (Address12 imm)
  | Lhu (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "lhu" rd rs1 (Address12 imm)
  | Sb (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "sb" rd rs1 (Address12 imm)
  | Sh (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "sh" rd rs1 (Address12 imm)
  | Sw (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "sw" rd rs1 (Address12 imm)
  | Beq (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "beq" rd rs1 (Address12 imm)
  | Bne (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "bnee" rd rs1 (Address12 imm)
  ...
  | Bltu (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "bltu" rd rs1 (Address12 imm)
  | Bgeu (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "bgeu" rd rs1 (Address12 imm)
  | Jalr (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "jalr" rd rs1 (Address12 imm)
  | Ld (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "ld" rd rs1 (Address12 imm)
  | Sd (rd, rs1, imm) -> pp_instruction_2reg_1offset_helper ppf "sd" rd rs1 (Address12 imm)
  | Addiw (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "addiw" rd rs1 (Address12 imm)
  | Slliw (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "slliw" rd rs1 (Address12 imm)
  | Srliw (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "srliw" rd rs1 (Address12 imm)
  | Sraiw (rd, rs1, imm) -> pp_instruction_2reg_1imm_helper ppf "sraiw" rd rs1 (Address12 imm)
  | Jal (rd, imm) -> Format.fprintf ppf "jal %s, %s" (pp_register rd) (pp_address (Address20 imm))
  | Jr (rs1) -> Format.fprintf ppf "jr %s" (pp_register rs1)
  | J (imm) -> Format.fprintf ppf "j %s" (pp_address (Address20 imm))
  | Lui (rd, imm) -> Format.fprintf ppf "lui %s,%s" (pp_register rd) (pp_address (Address20 imm))
  | Auipc (rd, imm) -> Format.fprintf ppf "auipc %s,%s" (pp_register rd) (pp_address (Address20 imm))
  | Ecall -> Format.fprintf ppf "ecall"
  | Call (str) -> Format.fprintf ppf "call %s" str
  | La (rd, imm) -> Format.fprintf ppf "la %s,%s" (pp_register rd) (pp_address (Address32 imm))
  | Lla (rd, imm) -> Format.fprintf ppf "lla %s,%s" (pp_register rd) (pp_address (Address32 imm))
  | Mv (rd, rs1) -> Format.fprintf ppf "mv %s,%s" (pp_register rd) (pp_register rs1)
  | Li (rd, imm) -> Format.fprintf ppf "mv %s,%s" (pp_register rd) (pp_address (Address32 imm))
...
let pp_directive ppf = function
  ...
  | Attribute (str, str_or_int) -> Format.fprintf ppf ".attribute %s %s" str (pp_str_or_int str_or_int)
  ...
  | TypeDir (str, str_type) -> Format.fprintf ppf ".type %s,%s" str (pp_type_dir str_type)
  ...
  | Section (str1, str2, str_type, none_or_int) -> 
    (match none_or_int with
     | Some imm -> Format.fprintf ppf ".section %s,%s,%s,%d" str1 str2 (pp_type_dir str_type) imm
     | None -> Format.fprintf ppf ".section %s,%s,%s" str1 str2 (pp_type_dir str_type))
  | String (str) -> Format.fprintf ppf ".string \"%s\"" str
